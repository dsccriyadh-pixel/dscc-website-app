...
try {
    dscc_file_mutate($file, ['recent' => [], 'days' => []], function (&$data) use ($day, $type, $label, $path, $src) {
        ...
        array_unshift($data['recent'], [
            'id' => 'e_' . base_convert((string) (int) (microtime(true) * 1000), 10, 36) . '_' . substr(bin2hex(random_bytes(4)), 0, 6),
            'ts' => gmdate('c'),
            'type' => $type,
            'label' => (string) $label,
            'path' => $path,
